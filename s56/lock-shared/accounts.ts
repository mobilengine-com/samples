...
//#using reftab account;
{
	...
	for (let row of tform.t.rows) {
		...
		if (rgth.Count() == 0) {
			let acc = {
				acc_num: row.recUpd.acc_num, holder_name: row.recUpd.holder_name, 
				valid: row.recUpd.valid, ver: 0
			};
			Log(["new account", acc]);

			db.account.Insert(acc);
			db.balance.Insert({acc_num: acc.acc_num, balance: row.recUpd.balance, ver: 0});
		}
	}
	...
	for (let row of tform.t.rows) {
		...
		if (th != null && b != null) {
			...
			if (accOld.holder_name != accUpd.holder_name || b.balance != row.recUpd.balance
				...
				if (row.rec.ver != accOld.ver) {
					...
					db.balance.Update({ acc_num: accUpd.acc_num }, {balance: row.recUpd.balance, ver: b.ver + 1});
				}
			}
		}
	}
